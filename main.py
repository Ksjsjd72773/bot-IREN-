...
def send_likes(uid):
    likes_api_response = requests.get(f"https://bngx-like-ff.onrender.com/add_likes?uid={uid}")
        ...
        while True:
            ...
            if senthi == True:
                ...
                clients.send(
                        self.GenResponsMsg(
                            f"""[C][B][1E90FF]╔═══════
[FFD700]مرحبا بكم في بوت IREN AND Noob
[1E90FF]╠
[FFFFFF]هل تريد بوت باسمك خاص تواصل مع مطوري البوت واسعار كسر لا تخاف
[FFFFFF]مطورين البوت: 
[FFD700]تيليجرام: @k_kwb and @noobbrmx
[1E90FF]╚══════""", idinv
                        )
                )
                senthi = False
...
[b][c][FFFFFF]----------------------------------
[C][B][FFD3EF]-Telegram :@k_kwb and @noobbrmx
[b][c][FF0000]-----------------------------------""",uid
                            )
                        )
                else:
...
[b][i][A5E2CFٍ]telegram: @k_kwb and @noobbrmx[/b]

[C][B][FF6347] Developpre IREN and Noob""", uid
                    )
                )
...
[A52A2A]________________________
Egyptian Developer:  
[00FF00]تم تطوير بوت بواسطة @noobbrmx and @k_kwb  """, uid
	                    )
	                )
	                time.sleep(0.5)
	                clients.send(
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="3852747561", password="747149DA8ADCA2B8ABCC6FBB3D60C9097235C56DDBFABA6ADBF7C5E13AEC2696")
        client_thread.start()
    except Exception as e:
