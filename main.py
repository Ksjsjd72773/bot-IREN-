#Egyptian developers: Muslim & AKIRA FF
#IG: @jak_moslem
#TElE: @zox_z8
#IG: @tmgx_kira
#TELE: @tmgx_kira
...
def newinfo(uid):
    try:
        url = f"https://aditya-info-v12op.onrender.com/player-info?uid={uid}&region=me"
        response = requests.get(url)
...
def send_spam(uid):
    try:
        ...
        api_url = f"https://rzx-team-api-spam-150.onrender.com/spam?id={uid}"
        response = requests.get(api_url)
        ...
        while True:
            ...
            if senthi == True:
                ...
                clients.send(
                        self.GenResponsMsg(
                            f"""[C][B][1E90FF]╔═══════
[FFFFFF]IREN and DRAGON-BOT welcomes you
[1E90FF]╠
[FFFFFF]هل تريد بوت باسمك خاص تواصل مع مطوري البوت واسعار كسر لا تخاف
[FFFFFF]مطورين البوت: 
[FFD700]تيليجرام: @k_kwb and @DRAGON_BYU
[1E90FF]╚══════""", idinv
                        )
                )
                senthi = False
...
[C][B][ffffff]- WeLCoMe To BOT IREN AND DRAGON-BOT
          [ffd319]        Good evening

[C][B][6E00FF]- هل رايت الاوامر؟ ارسل  :
...
[b][c][FF0000]-----------------------------------
[C][B][FFD3EF]-Telegram :@k_kwb and @DRAGON_BYU
[b][c][FF0000]-----------------------------------""",uid
                            )
                        )
                else:
...
[b][i][A5E2CFٍ]telegram: @k_kwb and @DRAGON_BYU[/b]

[C][B][FF6347] Developer IREN AND DRAGON-BOT  """, uid
                    )
                )
    ...
    f"[B][FFA500]• Bio: [FFFFFF]{bio}\n"
	            ...
	            if "1200" in data.hex()[0:4] and b"/hex" in data:
...
[C][B] [0000FF]☞/5 [id] -> [C][B][00FF00]فتح سكواد 5
[C][B] [0000FF]☞/6 [id] -> [C][B][00FF00]فتح سكواد 6
[C][B] [0000FF]☞/3 [id] -> [C][B][00FF00]فتح سكواد 3

[C][B] [0000FF]☞/attack -> [C][B][00FF00]]{{Tea CODE}}بدا اجباري

[C][B] [0000FF]☞/lag -> [C][B][00FF00]]{{Tea CODE}}لاق ثلاثة انواع
[C][B] [0000FF]☞/lag -> [C][B][00FF00]]{{Tea CODE}}2
[C][B] [0000FF]☞/lag -> [C][B][00FF00]]{{Tea CODE}}3""", uid
	                    )
	                )
	                time.sleep(0.5)
	                clients.send(
...
[C][B][0000FF]☞/inv {{uid}} -> [C][B][00FF00]جلب لاعب الى فريق عبر ايدي فقط

[C][B] [0000FF]☞/likes {{uid}} -> [C][B][00FF00]زيادة الايكات

[FFD700]/visit {{uid}} -> [C][B][00FF00]زيادة الزوار

[FFD700]/info {{uid}} [C][B][00FF00]جلب معلومات الحساب 

[FFD700]/x {{uid}} -> [C][B][00FF00]سبام طلبات انضمام

[FFD700]/check {{uid}} -> [C][B][00FF00]تحقق من باند

[FFD700]/status {{uid}} -> [C][B][00FF00]حالة اللاعب 

[FFD700]/dev {{uid}} -> [C][B][00FF00]معلومات مطور البوت

[FF0000]________________________
Egyptian Developer: IREN AND DRAGON-BOT  
[00FF00]تم تطوير بوت بواسطة IREN AND DRAGON-BOT  """, uid
	                    )
	                )
	                time.sleep(0.5)
	                clients.send(
...
[b][c][FF0000]🔥 اعضاء كلان  [b][c][00FF00]💚 KOKICLAN 
[b][c][FFFF00]✨ الى انضمام  [b][c][808000]لدخول الى كلان تواصل  [b][c][FF0000]📩 معي ⏬[b][c][FF00FF]TELEGRAM[ffffff]@k_kwb and @DRAGON_BYU
[C][B][00FFFF]━━━━━━━━""",uid
	                    )
	                )	            
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="3918554129", password="08C68151099C95427CCCFE2D63856F122EC3730415F76A3F485B9F1827905EE3")
        client_thread.start()
    except Exception as e:
