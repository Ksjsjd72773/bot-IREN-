...
def send_vistttt(uid):
    try:
        ...
        api_url = f"https://tmk-vist-v7.vercel.app/{uid}"
...
def attack_profail(player_id):
    url = f"https://tmk-vist-v7.vercel.app/{uid}"
...
class FF_CLIENT(threading.Thread):
    ...
    def spam_room(self, idroom, idplayer):
        ...
        2: {
            1: int(idroom),
            2: "iG:[C][B][FF0000]AK[FF00FF]I[00FFFF]RA[00FF00]_BOT",
            4: 330,
            5: 6000,
            6: 201,
...
[1E90FF]╚══════════════════════════╝""", idinv
            ...
            if "1200" in data.hex()[0:4]:
                if b"/spam" in data:
                    self.handle_spam_command(data)
                elif b"/room" in data:
                    self.handle_room_command(data)
                elif b"/stopspam" in data:
                    self.stop_spam()
                    json_result = get_available_room(data.hex()[10:])
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]
                    clients.send(
                        self.GenResponsMsg(
                            f"{generate_random_color()}تم إيقاف سبام الإنضمام",
                            uid))
                elif b"/stoproom" in data:
                    self.stop_room_spam()
                    json_result = get_available_room(data.hex()[10:])
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]
                    clients.send(
                        self.GenResponsMsg(
                            f"{generate_random_color()}تم إيقاف سبام الروم",
                            uid))
                elif b"/stoplag" in data:
                    if self.lag_thread and self.lag_thread.is_alive():
                        self.lag_thread.stop()
                        self.lag_thread.join()
                        clients.send(self.GenResponsMsg("[C][B][00FF00]Lag process stopped successfully.", uid))
                    else:
                        clients.send(self.GenResponsMsg("[C][B][FF0000]No active lag process to stop.", uid))
...
[b][i][A5E2CFٍ]telegram: @k_kwb [/b]

[C][B][FF6347] Developpre IREN """, uid
                    )
                )
            ...
            if "1200" in data.hex()[0:4] and b"WELCOME TO AKIRA FF BOT" in data:
            	pass
            else:
    ...
    f"[B][FFA500]• Bio: [FFFFFF]{bio}\n"
	            ...
	            if "1200" in data.hex()[0:4] and b"/help" in data:
	                ...
	                clients.send(
	                    self.GenResponsMsg(
		                        f"""[C][B][00FFFF]      ╔═━─═🔹═━━═╗
 [B][FFD3EF]IREN [ffd319]Ⓥ 
╚═━━═🔹═━━═╝

[C][B] [0000FF]☞/5 [id] -> [C][B][FFFFFF]فتح سكواد 5
[C][B] [FF69B4]☞/6 [id] -> [C][B][FFFFFF]فتح سكواد 6
[C][B] [000000]☞/3 [id] -> [C][B][FFFFFF]فتح سكواد 3

[C][B] [00FFFF]☞/attack -> [C][B][FFFFFF]]{{Tea CODE}}بدا اجباري

[C][B] [0000FF]☞/lag -> [C][B][FFFFFF]]{{Tea CODE}}لاق ثلاثة انواع
[C][B] [A52A2A]☞/lag -> [C][B][FFFFFF]]{{Tea CODE}}2
[C][B] [FFFFFF]☞/lag -> [C][B][FFFFFF]]{{Tea CODE}}3""", uid
	                    )
	                )
	                time.sleep(0.5)
	                clients.send(
...
[C][B][00FF00]☞/inv {{uid}} -> [C][B][00FFFF]جلب لاعب الى فريق عبر ايدي فقط

[C][B] [0000FF]☞/likes {{uid}} -> [C][B][00FFFF]زيادة الايكات

[A52A2A]/visit {{uid}} -> [C][B][00FFFF]زيادة الزوار

[FFD700]/info {{uid}} [C][B][00FFFF]جلب معلومات الحساب 

[00FFFF]/x {{uid}} -> [C][B][00FFFF]سبام طلبات انضمام

[000000]/check {{uid}} -> [C][B][00FFFF]تحقق من باند

[FFD700]/status {{uid}} -> [C][B][00FFFF]حالة اللاعب 

[F9F460]/[cُ][bَ]spam <id>[FE0202]سبام لا نهائي

[EDFF07]/َstopspam <id [00FF00]ازالة سبام لا نهائي

[F9F460]/[F9F460]CH <id>[FE0202]ارسال خمسين سبام

[EDFF07]/َGEME[00FF00] لاغ  في القيم

[EDFF07]/ranked [team code][00FF00]دخول مع بوتات في رانكد
[A52A2A]________________________ """, uid
	                    )
	                )
	                time.sleep(0.5)
	                clients.send(
...
[C][B][FF69B4]━━━━━━━━""",uid
            ...
            if '1200' in data.hex()[0:4] and b'/GEME' in data:
                try:
                    split_data = re.split(rb'/GEME', data)
                    command_parts = split_data[1].split(b'(')[0].decode().strip().split()
            ...
            if "1200" in data.hex()[0:4] and b"/ranked" in data:
                try:
                    split_data = re.split(rb'/ranked', data)
                    command_parts = split_data[1].split(b'(')[0].decode().strip().split()
            ...
            if "1200" in data.hex()[0:4] and b"/sm" in data:
                try:
                    ...
                    for _ in range(50):
                        ...
                        sent_inv = True
                        sent_count += 1
                        logging.info(f"Squad request {sent_count} sent to ID: {sid}")
                        time.sleep(0.2)  # تأخير 0.5 ثانية بين كل إرسال

                    # إرسال رسالة تأكيد بعد اكتمال الإرسال
                    clients.send(
                        self.GenResponsMsg(
                            f"[C][B][00ff00]Squad request sent to ID: {fix_num(sid)} {sent_count} times successfully!", uid
                        )
                    )
                except Exception as e:
                    logging.error(f"Error processing /CH {e}")
                    clients.send(
                        self.GenResponsMsg(
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="3966541351", password="92E4370C24915AA8DB8D15ABBC9876912F7A2140C773796C7ECC66259B264FF3")
        client_thread.start()
    except Exception as e:
